...
class _GCSBlobReader(io.BytesIO):
    def __init__(self, blob):
        self._blob = blob
        super().__init__(blob.download_as_bytes())

class _GCSBlobWriter(io.BytesIO):
    def __init__(self, blob):
        self._blob = blob
        super().__init__()
...
def _open(path, mode, bucket=None):
    ...
    if 'r' in mode:
        return _GCSBlobReader(blob)
    if 'w' in mode:
        return _GCSBlobWriter(blob)
...
class MultiFileReader:
    ...
    def read(self, locs, n_bytes):
        ...
        for f_name, offset in locs:
            f_name = f_name if str(f_name).startswith(str(self._base_dir)) else str(self._base_dir / f_name)
